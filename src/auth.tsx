...
function AuthState({ children, authConfigured, apiScope }: { children: ReactNode; authConfigured: boolean; apiScope: string | undefined }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await fetch(`${apiBaseUrl}/api/v1/me`, { headers: { Authorization: `Bearer ${breakglassToken}` } });
        if (ignore) return;
        if (!response.ok) { clearStoredBreakglassToken(); setBreakglassTokenState(null); setBreakglassChecking(false); return; }
        ...
        if (!ignore) setBreakglassChecking(false);
      }
    })();
    return () => { ignore = true; };
  ...
  return <AuthContext.Provider value={{ role: (authenticated || breakglassActive) ? role : 'user', account, loading: inProgress !== 'none' || apiLoading || breakglassChecking, signIn, signOut, apiRequest, authConfigured, breakglassActive, breakglassUsername, breakglassElevated, idpUnreachable, elevateBreakglass }}>{children}</AuthContext.Provider>;
}
