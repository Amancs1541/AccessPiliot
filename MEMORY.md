...
## Phase 4 — Recurring Sync Scheduler

Added a real in-process background scheduler (no new infra — no Celery/Redis, since none is configured): `identity_providers.sync_interval_minutes` (migration `0004`), a poll loop in `app/workers/scheduler.py` (checks every 60s whether a provider's schedule is due, based on `last_sync_at`), wired into `app/main.py` via a `lifespan` context manager (replaced the deprecated `@app.on_event` pattern) alongside the (also new) expiration worker. Disabled when `ENVIRONMENT=test` so it never runs during the test suite. Reused the existing `PATCH /api/v1/providers/{id}` endpoint for setting the schedule — no new route. Sync page UI has a "Scheduled sync" panel (interval input + Save/Disable). 5 tests added.

## Phase 4/5 — Assignment & Approval + Real Entra Provisioning

New feature (admin assigns GROUP/ROLE access to a user, optional approver, optional time-bound expiration), built entirely on the pre-existing `access_assignments` table — no new tables, no migration.

- `app/services/assignments.py`: create/list/get/approve/reject. Statuses used: `PENDING_APPROVAL`, `ACTIVE`, `REJECTED`, `EXPIRED` (a documented decision — the original state-machine doc's assignment vocabulary (PENDING/ELIGIBLE/ACTIVE) didn't include an approval-decision state; this feature needed one, so `PENDING_APPROVAL`/`REJECTED` were added, matching the access_request vocabulary instead).
- **Real Entra provisioning is wired to the actual state transitions, not just DB writes**: creating an assignment with no approver, or approving one, calls `EntraProvider.activate_assignment()` (which now does a REAL Graph call: `POST /groups/{id}/members/$ref` for GROUP, `POST /directoryRoles/{id}/members/$ref` for ROLE) *before* the row is ever marked/persisted as ACTIVE. If the Graph call fails, the assignment is NOT created (for immediate-activation) or NOT flipped to ACTIVE (for approval) — matches the state-machine doc's "provider failures must not silently create false ACTIVE state." A 409/already-exists response from Graph is treated as idempotent success.
- `app/workers/expiration.py` was a no-op stub before this phase; it's now a real poll loop (same lifespan pattern as the sync scheduler) that calls `EntraProvider.revoke_assignment()` (`DELETE .../members/{id}/$ref`) for due time-bound assignments *before* marking them `EXPIRED`. If the Graph removal fails, the assignment is left `ACTIVE` and retried on the next poll — never falsely marked expired.
- Permissions gated via the *existing* `ASSIGNMENT_READ`/`ASSIGNMENT_CREATE` Admin permissions — no new permission strings invented. There is no separate "Approver" role (per `04_FINAL_APP_ROLES.md`, explicitly a V2-only candidate); any Admin can approve/reject any pending assignment — the designated `approver_id` is stored and shown for routing/audit purposes but isn't an enforced identity check beyond the existing Admin authorization.
- **Known permission gap, not yet verified**: role assignment write (`POST/DELETE /directoryRoles/{id}/members`) requires Graph application permission `RoleManagement.ReadWrite.Directory`. Only `RoleManagement.Read.Directory` was previously confirmed granted. Group membership write uses `Group.ReadWrite.All`, which **was** already confirmed granted. Until `RoleManagement.ReadWrite.Directory` is granted+consented, ROLE-target assignments will fail safely with `PROVIDER_PERMISSION_DENIED` on activation/approval — GROUP-target assignments should work today.
- Frontend: Assignments page rebuilt from mock data to real API calls — "Add assignment" dialog (user/target-type/target/duration/approver/justification), Approve/Reject buttons on `PENDING_APPROVAL` rows.
- Tests: 21 new (10 core assignment/approval flow, 8 real Graph group/role provisioning calls via `httpx.MockTransport`, 3 provider-failure safety). Full suite: 83/83 passing. Frontend build passing.
- Not implemented (not requested this round): revoke/extend UI for already-ACTIVE assignments, a dedicated Approver role, self-service access requests — all remain out of scope.
